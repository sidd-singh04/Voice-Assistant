...
const App = () => {
  ...
  // Unlock TTS on first user gesture (needed for mobile)
  const unlockTTS = () => {
    if (!ttsUnlocked) {
  ...
  const startListening = () => {
    unlockTTS(); // unlock speech for mobile
    recognition.start();
    setIsListening(true);
  };
  ...
  const speakText = (text) => {
    if (!ttsUnlocked) {
      console.warn("TTS not unlocked yet. Tap button to activate.");
      return;
    }
    ...
    // Select first English voice (simplified for mobile)
  ...
  const handleVoiceCommand = async (command) => {
    ...
    if (command.includes("introduce")) {
      const response =
        "Hello Sir, I'm Friday, your personal voice assistant created by Siddharth to help you with tasks using speech commands.";
      speakText(response);
      setInformation(response);
      return;
    }
